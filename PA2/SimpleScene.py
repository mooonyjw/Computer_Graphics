...
initialPosition = np.zeros(3) # 3D 벡터로 초기화
cowPosition = np.identity(4) # 4x4 단위 행렬로 초기화
...
def drawCow(_cow2wld, drawBB):
    glPushMatrix()  # Push the current matrix of GL into stack. This is because the matrix of GL will be changed while drawing cow.
...
def display():
    global cameraIndex, cow2wld, count, positions, initialPosition, cowPosition, AnimationStarted, startTime, isDrag, isNotInitialClick
...
def onMouseDrag(window, x, y):
    ...
    if isDrag:
        ...
        if isDrag == V_DRAG:
            ...
            # 수평 드래그
            if cursorOnCowBoundingBox:
                plane_normal = np.array((0, 1, 0))  # 수평 평면
                cow_translation = getTranslation(cow2wld)
                p = Plane(plane_normal, cow_translation)
                intersect_result = ray.intersectsPlane(p)
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, '2022066953-limjungwoni', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
